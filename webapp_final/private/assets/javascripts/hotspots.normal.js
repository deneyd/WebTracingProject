...
function renderHotspot(hotspots, user) {
  hotspots.forEach(function(hotspot, index) {
    ...
    if (user.address) {
      vm.hasAddress = true;
      hotspot.distanceToHome = turf.distance(turf.point(user.address.location), turf.point(hotspot.address.location));
    }
  });
  if (user.address) {
    user.popup = new mapboxgl.Popup({
      closeButton: false,
    })
    .setHTML(`
      <div>
        <h4 class="text-center text-white bg-primary">Your House</h4>
        <p>Address: ${user.address.formatted}</p>
      </div>
    `);
    let el = document.createElement('div');
    el.className = 'marker';
    user.marker = new mapboxgl.Marker(el, {
      color: '#FFFFFF',
    }).setLngLat(user.address.location).setPopup(user.popup).addTo(map);
  }
}
        ...
        xhttp.onreadystatechange = function() {
          if (this.readyState == 4) {
            if (this.status == 200) {
              ...
              renderHotspot(vm.hotspots, vm.user);
            } else {
              vm.status = false;
              return;
            }
